...
const BOWLING_COLS = [
  ...
  { key: 'wides', label: 'WD' },
  { key: 'no_balls', label: 'NB' },
] as const;

type View = 'batting' | 'bowling';
...
export default function CricketPlayersTab({ fixture, result, homeClub, awayClub }: CricketPlayersTabProps) {
  ...
  // Get unique innings numbers
  const inningsNumbers = useMemo(() => {
    if (!inningsData) return [1];
    const nums = [...new Set(inningsData.map((i: any) => i.innings_number))].sort();
    return nums.length > 0 ? nums : [1];
  }, [inningsData]);

  // Filter stats for selected team + innings
  ...
  // Batters: those who faced balls or scored runs
  const batters = useMemo(() => {
    return filteredStats
      .filter((s: any) => (s.balls_faced ?? 0) > 0 || (s.runs_scored ?? 0) > 0 || s.how_out)
      .sort((a: any, b: any) => {
        // Sort by batting order (approx: those who scored more faced more balls first)
        return 0; // Keep insertion order
      });
  }, [filteredStats]);

  // Bowlers: those who bowled overs
  const bowlers = useMemo(() => {
    // For bowling, we want the opposing team's stats
  ...
  // Get innings result for this team
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="match-card p-2 flex flex-col gap-2">
      ...
      <div className="match-card overflow-hidden p-1 flex gap-1">
        <button
          ...
          className={`flex-1 py-2 text-xs font-bold rounded-md transition-all ${
            view === 'batting' ? 'bg-primary text-primary-foreground shadow-sm' : 'text-muted-foreground hover:text-foreground hover:bg-muted/50'
          }`}
        >
          Batting
        </button>
        <button
          ...
          className={`flex-1 py-2 text-xs font-bold rounded-md transition-all ${
            view === 'bowling' ? 'bg-primary text-primary-foreground shadow-sm' : 'text-muted-foreground hover:text-foreground hover:bg-muted/50'
          }`}
        >
          Bowling
        </button>
      ...
      {view === 'batting' && (
        <div className="match-card overflow-hidden">
          <div className="px-4 py-3 border-b border-border/30">
            <h3 className="text-xs font-black uppercase tracking-wider text-muted-foreground">Batting Scorecard</h3>
          </div>
          {batters.length > 0 ? (
              ...
              <table className="w-full text-[10px]">
                <thead>
                  <tr className="border-b border-border/30">
                    <th className="text-left py-2 px-2 font-black uppercase tracking-wider text-muted-foreground sticky left-0 bg-card z-10 min-w-[140px]">Batter</th>
                    <th className="text-left py-2 px-1.5 font-bold text-muted-foreground min-w-[80px]">How Out</th>
                    {BATTING_COLS.map(col => (
                      <th key={col.label} className="text-center py-2 px-1.5 font-black uppercase tracking-wider text-muted-foreground min-w-[32px]">
                        {col.label}
                      </th>
                    ))}
                  </tr>
                          ...
                          <div className="flex items-center gap-1.5">
                            ...
                            <div className="min-w-0">
                              <span className="font-bold text-[10px] truncate block">
                                ...
                                {s.not_out && <span className="text-accent ml-0.5">*</span>}
                              </span>
                            </div>
                          </div>
                        </td>
                        <td className="py-1.5 px-1.5 text-[9px] text-muted-foreground truncate max-w-[80px]">
                          {s.not_out ? 'not out' : s.how_out || '-'}
                        </td>
                        <td className={`text-center py-1.5 px-1.5 font-bold ${(s.runs_scored ?? 0) >= 50 ? 'text-accent' : (s.runs_scored ?? 0) >= 30 ? 'text-primary' : ''}`}>
                          {s.runs_scored ?? 0}
                        </td>
                        <td className="text-center py-1.5 px-1.5 text-muted-foreground">{s.balls_faced ?? 0}</td>
                        <td className="text-center py-1.5 px-1.5">{s.fours ?? 0}</td>
                        <td className="text-center py-1.5 px-1.5">{s.sixes ?? 0}</td>
                        <td className="text-center py-1.5 px-1.5 tabular-nums">{Number(s.strike_rate ?? 0).toFixed(1)}</td>
                      </tr>
                    );
                  })}
                </tbody>
            ...
            <div className="px-4 py-6 text-center">
              <p className="text-xs text-muted-foreground/50">No batting data recorded</p>
            </div>
          )}
        </div>
      )}
      ...
      {view === 'bowling' && (
        <div className="match-card overflow-hidden">
          <div className="px-4 py-3 border-b border-border/30">
            <h3 className="text-xs font-black uppercase tracking-wider text-muted-foreground">Bowling Figures</h3>
          </div>
          {bowlers.length > 0 ? (
            <ScrollArea className="w-full">
              <table className="w-full text-[10px]">
                <thead>
                  <tr className="border-b border-border/30">
                    <th className="text-left py-2 px-2 font-black uppercase tracking-wider text-muted-foreground sticky left-0 bg-card z-10 min-w-[130px]">Bowler</th>
                      ...
                      <tr key={s.id} className={i % 2 === 0 ? 'bg-muted/20' : ''}>
                        <td className="py-1.5 px-2 sticky left-0 z-10" style={{ backgroundColor: i % 2 === 0 ? 'hsl(var(--muted) / 0.2)' : 'hsl(var(--card))' }}>
                          <div className="flex items-center gap-1.5">
                            ...
                            <span className="font-bold text-[10px] truncate">{s.players?.first_name?.[0]}. {s.players?.last_name}</span>
                          </div>
                        </td>
                        <td className="text-center py-1.5 px-1.5">{Number(s.overs_bowled ?? 0).toFixed(1)}</td>
                        <td className="text-center py-1.5 px-1.5">{s.maidens ?? 0}</td>
                        <td className="text-center py-1.5 px-1.5">{s.runs_conceded ?? 0}</td>
                        <td className={`text-center py-1.5 px-1.5 font-bold ${(s.wickets ?? 0) >= 5 ? 'text-accent' : (s.wickets ?? 0) >= 3 ? 'text-primary' : ''}`}>
                          {s.wickets ?? 0}
                        </td>
                        <td className="text-center py-1.5 px-1.5 tabular-nums">{Number(s.economy ?? 0).toFixed(1)}</td>
                        <td className="text-center py-1.5 px-1.5 text-muted-foreground">{s.wides ?? 0}</td>
                        <td className="text-center py-1.5 px-1.5 text-muted-foreground">{s.no_balls ?? 0}</td>
                      </tr>
                    );
                  })}
                </tbody>
            ...
            <div className="px-4 py-6 text-center">
              <p className="text-xs text-muted-foreground/50">No bowling data recorded</p>
            </div>
          )}
        </div>
      )}
