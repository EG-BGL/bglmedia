...
export default function SubmitResult() {
  ...
  useEffect(() => {
    ...
    const loadFixtures = async () => {
      // Get the coach's assigned team IDs (for both coaches and admins acting as coaches)
      ...
      if (role === 'league_admin' && teamIds.length === 0) {
        // Admin with no coach assignments — show all fixtures
        const { data } = await supabase.from('fixtures').select('*, home_team:teams!fixtures_home_team_id_fkey(*, clubs(*)), away_team:teams!fixtures_away_team_id_fkey(*, clubs(*))').in('status', ['scheduled', 'in_progress']).order('round_number');
        setFixtures(data ?? []);
        ...
        const { data } = await supabase.from('fixtures').select('*, home_team:teams!fixtures_home_team_id_fkey(*, clubs(*)), away_team:teams!fixtures_away_team_id_fkey(*, clubs(*))').or(teamIds.map(id => `home_team_id.eq.${id},away_team_id.eq.${id}`).join(',')).in('status', ['scheduled', 'in_progress']).order('round_number');
        setFixtures(data ?? []);
      }
    };
    loadFixtures();
      ...
      <AlertDialog open={showSuccess}>
        <AlertDialogContent className="sm:max-w-md">
          <AlertDialogHeader className="text-center items-center">
            ...
            <AlertDialogTitle className="text-lg font-black">Result Submitted!</AlertDialogTitle>
